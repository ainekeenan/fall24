    ...
    if youtube_link:
        ...
        if len(cv) != 0:
            ...
            try:
                ...
                command = f"Do not use markdown or any fancy formatting, only pure text. Reply in complete full sentences, no bullet points. Strictly only reply in bullet points with concepts that you think I would enjoy if I enjoyed the text below, {transcript_text}."
                response = convo.send_message(command)
                summary = response.text
